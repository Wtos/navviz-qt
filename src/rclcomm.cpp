...
rclcomm::rclcomm()
{
    ...
    node=rclcpp::Node::make_shared("navviz_qt");
...
//            qDebug() << "Before transform:" << x << y << "; After transform:" << point_map_frame.point.x << point_map_frame.point.y;
...
void rclcomm::globalCostMapCallback(
  ...
  // Flip vertically: occupancy grid origin is bottom-left, QImage origin is top-left
  map_image = rotateMapWithY(map_image);
  //  map_image.save("<output-path>/global_costmap.png");  // example
...
void rclcomm::localCostMapCallback(
  ...
  try {
      // Transform local costmap pose into map frame for rendering
      ...
      double roll,pitch,yaw;
      mat.getRPY(roll,pitch,yaw);
      // Convert origin to scene coordinates
...
void rclcomm::map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg){
   double origin_x = msg->info.origin.position.x;
   double origin_y = msg->info.origin.position.y;
   qDebug() << "Received /map. origin_x:" << origin_x << " origin_y:" << origin_y;
   int width =msg->info.width;
   int height =msg->info.height;
...
//   map_image.save("<output-path>/map.png");
   QImage rotate_map=rotateMapWithY(map_image);
//   rotate_map.save("<output-path>/rotate_map.png");
   emit emitUpdateMap(rotate_map);
   // Compute scene origin in world coordinates (after vertical flip)
   double trans_origin_x = origin_x;
   double trans_origin_y = origin_y+height*m_resolution;
   // Compute world origin in scene coordinates
...
//     qDebug()<<msg->data;
     emit emitTopicData("Received message: " + QString::fromStdString(std::to_string(msg->data)));
}
