...
void roboItem::drawTools(QPainter* painter){
    ...
    if (m_currCursor == m_set2DPoseCursor || m_currCursor == m_set2DGoalCursor) {
      ...
      if (m_startPose.x() != 0 && m_startPose.y() != 0 &&
        ...
        // Compute arrow direction (radians)
        ...
        point1.setX(endPoint.x() + 10 * cos(angle - 0.5)); // arrow head point 1
        point1.setY(endPoint.y() + 10 * sin(angle - 0.5));
        point2.setX(endPoint.x() + 10 * cos(angle + 0.5)); // arrow head point 2
...
void roboItem::drawGlobalCostMap(QPainter *painter) {
  // Render method 1: draw image directly
  painter->save();
...
//  painter->rotate(rad2deg(-m_lastLocalCostMapRobotPose.theta));
  painter->drawImage(
      0, -m_LocalCostMap.height(),
      m_LocalCostMap);
  painter->restore();
  // Render method 2: draw points
  //        painter->save();
...
void roboItem::wheelEvent(QGraphicsSceneWheelEvent *event) {
    ...
    // Emulate zoom anchored under mouse position
    if(event->delta()>0){
